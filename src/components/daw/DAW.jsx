...
const DAW = ({ onContentChange }) => {
  ...
  const generateContent = async () => {
    ...
    try {
      ...
      // Notificar o componente pai sobre a mudança de conteúdo
  ...
  const downloadAudio = async () => {
    ...
    try {
      ...
      // Gerar áudio da voz
      setProgress(30);
      const voiceAudio = await generateSpeech(aiSuggestion, selectedVoice, selectedLanguage);

      // Mixar com a trilha
      setProgress(70);
      const mixedAudio = await mixAudio(voiceAudio, selectedTrack);

      // Download
      setProgress(100);
      const audioBlob = await fetch(mixedAudio).then(r => r.blob());
