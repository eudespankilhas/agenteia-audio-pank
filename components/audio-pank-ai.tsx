...
import { AlertTriangle, X, Mic, Square, Send, Volume2, Sliders } from "lucide-react"
...
export default function AudioPankAI() {
  ...
  const sendMessage = async (messageText?: string) => {
    ...
    try {
      ...
      if (data.result) {
        const aiResponse = { role: "assistant", content: data.result, criado_em: new Date() }
        ...
        <TabsContent value="chat" className="space-y-4">
          ...
          <div className="mt-4">
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <button
                className="rounded-md bg-gray-200 p-2 text-sm hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500"
                onClick={() => handleTopicSelection("Vamos saber mais sobre futebol?")}
              >
                Como funciona o plano de assinatura?
              </button>
              <button
                className="rounded-md bg-gray-200 p-2 text-sm hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500"
                onClick={() => handleTopicSelection("Quer saber sobre programas de tv?")}
              >
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 mt-2">
              <button
                className="rounded-md bg-gray-200 p-2 text-sm hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500"
                onClick={() => handleTopicSelection("Conhecer prais no Brasil?")}
              >
          ...
          <div className="flex gap-2 mt-4">
            <input
              type="text"
              className="flex-grow rounded-md border p-2 dark:bg-gray-800 dark:text-white"
            ...
            />
            <button
              className="rounded-md bg-blue-500 p-2 text-white hover:bg-blue-600 disabled:bg-blue-300 flex items-center justify-center"
              onClick={() => sendMessage()}
              disabled={isLoading || !input.trim()}
            >
              {isLoading ? <span className="animate-pulse">Enviando...</span> : <Send className="h-5 w-5" />}
            </button>
          </div>
